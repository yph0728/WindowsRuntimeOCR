...
namespace OCR
{
...
#endif
    {
        ...
        private async Task<StorageFile> ScaleImageAync(StorageFile file)
        {
            var imageResizer = new ImageResizer(new SizeValidator(40,2600));

            imageResizer.EnsureSuccesfullScale();

            ImageResult result = await imageResizer.ResizeAsync(file);

            file = result.File as StorageFile;
            return file;

        }
        public StorageFile file { get; set; }

        //private async void load_RealestateImage(object sender, RoutedEventArgs e)
        //{
        //    //ImageProperties imgProp = await file.Properties.GetImagePropertiesAsync();
        //    var picker = new FileOpenPicker()
        //    {
        ...
        private async void RealEstate_Click(object sender, RoutedEventArgs e)
        {

            var newfile = await ScaleImageAync(file);
